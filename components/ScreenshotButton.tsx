...
export default function ScreenshotButton() {
  ...
  const takeScreenshot = async () => {
    ...
    try {
      ...
      // Find the visible preview container and get the template root inside it
  ...
  return (
    <motion.button
      ...
      whileTap={{ scale: 0.95 }}
      className="flex items-center gap-1.5 px-3 py-2 rounded-lg bg-white/[0.04] hover:bg-white/[0.08] border border-white/[0.06] text-xs text-white/50 hover:text-white/80 transition disabled:opacity-50"
    >
      {state === 'loading' ? (
