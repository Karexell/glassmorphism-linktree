...
export default function Home() {
  ...
  return (
    <div className="h-[100dvh] flex flex-col bg-[#050010]">
      {/* Top Bar */}
      <motion.header
        initial={{ opacity: 0, y: -10 }}
        animate={{ opacity: 1, y: 0 }}
        className="flex items-center justify-between px-4 sm:px-5 py-2.5 sm:py-3 border-b border-white/[0.06] bg-white/[0.02] backdrop-blur-xl z-20 flex-shrink-0"
      >
        <div className="flex items-center gap-2 sm:gap-3">
          <motion.button
            ...
            whileTap={{ scale: 0.95 }}
            className="p-1.5 sm:p-2 rounded-lg bg-white/[0.04] hover:bg-white/[0.08] border border-white/[0.06] transition"
          >
            {panelOpen ? (
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs sm:text-sm font-semibold text-white/80">Glass Links</span>
          </div>
        </div>
      ...
      <div className="sm:hidden px-4 py-2 border-b border-white/[0.04] bg-white/[0.01]">
        <TemplateSelector
          selected={data.templateId as TemplateId}
          onSelect={(id) => setData({ ...data, templateId: id })}
        />
      ...
      <div className="flex flex-1 overflow-hidden min-h-0">
        ...
        <AnimatePresence mode="wait">
          {panelOpen && (
            <motion.aside
              ...
              className="border-r border-white/[0.06] bg-white/[0.01] overflow-hidden flex-shrink-0 hidden sm:block"
            >
        ...
        {/* Mobile editor (below preview on small screens) */}
        <div className="sm:hidden flex-1 flex flex-col overflow-hidden min-h-0">
          ...
          <div className="h-[45vh] border-t border-white/[0.06] overflow-hidden">
            <EditorPanel data={data} onChange={setData} />
          </div>
        </div>
